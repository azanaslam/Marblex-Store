...
export const CatalogsPage = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto min-h-screen pb-16">
      <div className="px-4 text-center mb-8 md:mb-12 mt-4">
      ...
      <div className="md:hidden sticky top-[68px] z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 mb-6 px-4 py-3 -mx-4 overflow-x-auto scrollbar-hide flex items-center gap-3 snap-x">
        {catalogs.map((cat) => (
          <button
            key={cat.id}
            onClick={() => setActiveCatalog(cat.id)}
            className={`flex-shrink-0 snap-start px-6 py-2.5 rounded-full text-sm font-bold transition-all duration-300 ${
      ...
      <div className="flex flex-col md:flex-row gap-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:block w-full md:w-1/3 lg:w-1/4 flex-shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
            <div className="p-5 md:p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
              <div>
                <h2 className="text-xl md:text-3xl font-black text-slate-900">{selectedCatalog.title}</h2>
                <p className="text-sm text-slate-500 font-medium mt-1">{selectedCatalog.pages.length} Pages</p>
              </div>
            </div>
            
            <div className="p-3 sm:p-8 bg-slate-100">
              <div className="max-w-4xl mx-auto space-y-4 md:space-y-8">
                {selectedCatalog.pages.map((pageImg, idx) => (
                  <div key={idx} className="relative group rounded-xl md:rounded-3xl overflow-hidden shadow-md border border-slate-200 bg-white">
                    <img 
                      src={pageImg} 
                      ...
                      loading="lazy"
                    />
                    <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-sm text-white text-xs font-bold px-3 py-1.5 rounded-lg opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                      Page {idx + 1}
                    </div>
                  </div>
                ))}
